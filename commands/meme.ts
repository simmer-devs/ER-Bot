...
export default class meme implements IBotCommand{
    ...
    async runCommand(args: string[], msgObject: Discord.Message, client: Discord.Client): Promise<void> {
        ...
        if(msgObject.channel.id === "349789282447130636"){
            ...
            let msg = await msgObject.channel.send("Gathering memes...") as Discord.Message

            fetch("https://apis.duncte123.me/meme")
            .then((res: any) => res.json())
            .then((body: any) => {
                if(!body || !body.data.image) {return msgObject.reply("Whoops! Try again.").then(msg => {(msg as Discord.Message).delete(3000)})}

                let memebed = new Discord.RichEmbed
                memebed.setColor("DARK_GOLD")
                memebed.setAuthor(`${client.user.username} Meme`, msgObject.guild.iconURL)
                memebed.setImage(body.data.image)
